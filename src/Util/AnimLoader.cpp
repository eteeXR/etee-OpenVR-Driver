...
void GLTFModelManager::LoadKeyframeTimes() {
  ...
  DebugDriverLog("Keyframe count: %i", keyframeTimes_.size());
...
// Used for finger curl
AnimationData GLTFModelManager::GetAnimationDataByInterp(const HandSkeletonBone& boneIndex, const float f) const {
...
// Used for accessory animations
AnimationData GLTFModelManager::GetAnimationDataByKeyframes(const HandSkeletonBone& boneIndex, const int firstKeyframe, const int lastKeyframe) const {
  AnimationData result;

  result.startTransform = keyframeTransforms_[static_cast<size_t>(boneIndex)][firstKeyframe];
  result.startTime = keyframeTimes_[firstKeyframe];
  result.endTransform = keyframeTransforms_[static_cast<size_t>(boneIndex)][lastKeyframe];
  result.endTime = keyframeTimes_[lastKeyframe];
